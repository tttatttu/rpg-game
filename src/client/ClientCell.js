...
class ClientCell extends PositionedObject {
  ...
  initGameObjects() {
    ...
    this.objects = cellCfg.map((layer, layerId) =>
      layer.map((objCfg) =>
        new ClientGameObject({ cell: this, objCfg, layerId })));
  }
  ...
  addGameObject(objToAddProps) {
    const objToAdd = objToAddProps;
    const { objects } = this;
  ...
  removeGameObject(objToRemove) {
    ...
    objects.forEach((layer, layerId) =>
      (objects[layerId] = layer.filter((obj) =>
        obj !== objToRemove)));
  }
  ...
  findObjectsByType(type) {
    ...
    this.objects.forEach((layer) =>
      (foundObjects = [...foundObjects, ...layer].filter((obj) =>
        obj.type === type)));
    return foundObjects;
  }
}
